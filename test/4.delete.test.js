...
const Mock = require('./mock/delete')
...
describe('Task Delete Tests', () => {
  ...
  it('Expect to not delete an task because task not found', done => {
    try {
  ...
  it('Expect to delete an task', done => {
    try {
      const pattern = Mock.pattern
      ...
      seneca.act(pattern, payload, (err, response) => {
        ...
        expect(typeof response.result).to.be.equal('number')
        expect(response.result).to.be.equal(1)
        done(null)
      })
    } catch (err) {
      done(err)
